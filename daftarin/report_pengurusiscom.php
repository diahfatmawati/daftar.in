...
$sheet->setCellValue('C2', 'Cetak Data Pendaftar Pengurus ISCOM');
...
$sheet->setCellValue('R4', 'Divisi 1');
$sheet->setCellValue('S4', 'Divisi 2');
$sheet->setCellValue('T4', 'Alasan memilih Divisi');
$sheet->setCellValue('U4', 'Alasan mendaftar ISCOM');
 
$query = mysqli_query($koneksi,"select * from anggota_iscom");
$i = 5;
$no = 1;
while($row = mysqli_fetch_array($query))
{
	...
	$sheet->setCellValue('R'.$i, $row['div1']);
	$sheet->setCellValue('S'.$i, $row['div2']);
	$sheet->setCellValue('T'.$i, $row['alasan_div']);	
	$sheet->setCellValue('U'.$i, $row['alasan_iscom']);		
	$i++;
}
$styleArray = [
			'borders' => [
...
$writer->save('Report Pendaftar Pengurus ISCOM.xlsx');
?>